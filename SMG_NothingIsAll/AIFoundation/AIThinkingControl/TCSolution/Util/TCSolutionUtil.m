    ...
    for (TOFoModel *rCansetFromModel in rCansets) {
        ...
        NSArray *cansetFroms3 = [SMGUtils convertArr:cansetFroms2 convertBlock:^id(AIKVPointer *hCansetFrom) {
            ...
            for (NSInteger i = hCansetCutIndex + 1; i < obj.transferXvModel.cansetToOrders.count; i++) {
                ...
                //  注意、不允许H失败时，hAlg传染整个工作记忆树了，它只传染当前IF树，因为当前IF树没有它的H解，不表示别的IF树也没有它的H解。
                //  示例佐证、当你在陌生城市饿了时，把这里识别成室内和城市两个IF场景：室内时你不能使用别人的厨房获取食物。但城市时你可以点外卖获取食物。
                //8、TODOLIST
                //  TODO1、写hSolutionV4还是依赖r迁移关联来求h解。
                //  TODO2、单个IF场景H求解失败时，只在IF场景内传染 `已否认，不必改 T`。
                ...
                //  TODO4、那怎么办呢？有中间帧的目标所在的h解，是无法迁移过来的（那么我们可以捋一下现在的整个流程，看它能不能从无H解到有H解，并且跑通）。
                //      步骤1、该传染就传染，传染后自然能激活别的未传染掉的rCanset。
