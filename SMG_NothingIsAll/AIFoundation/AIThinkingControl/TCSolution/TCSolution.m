...
+(void) solution{
    ...
    //----------TODOTOMORROW20211208: solution代码规划 (参考24186);
    //1. 有子任务待决策时: 子任务决策优先级 (参考24185);
    //  a. 取出所有S;
    //  b. 根据>cutIndex筛选 (参考24185-方案1-代码);
    //  c. 把待决策的筛选出来 (参考24185-方案1-代码);
    //  d. 对其进行baseR迫切度竞争;
    //  e. 最终筛选有结果时,取首条进行子任务决策;
    //  f. 最终筛选无结果时,转最优路径;
    
    
    //2. 无子任务待决策时: 短记忆树最优路径 (参考24184);
    
    //3. 改为末评pk,取出所有末尾评分,直接排序后对首个进行决策;
