...
+(TCResult*) hSolution:(HDemandModel*)hDemand{
    ...
    //1. 只要hSolution执行了,它就有踢的经验被取到;
    if ([NVHeUtil algIsYouPiGuo:hDemand.baseOrGroup.content_p]) {
        for (TOFoModel *item in hDemand.actionFoModels) {
            NSLog(@" --> %@",Pit2FStr(item.cansetFrom));
        }
    }
    
    //2. 但有皮果动机后,似乎并不总是能执行到hSolution,看起来像TCScore竞争的问题,它没继续执行到;
    //测了下,看起来现在的TCScore+TCPlan运行ok;
    //下面的Solution可能是由HDemand或者Input P调用起来的,而不是TO循环在调用;
    
    //=============================== 9 hDemand ===============================
    //flt1 A3955(向90,距13,皮果)
    //TO上轮:成功 等待:0.0 下轮:6 消息:out 此帧需要HDemand来完成,已转为h任务
    //第1条 饿 评分-11.25 激活成功     {proto:F9096 pFos:(F3900,F3871,F3825,F3717,F3566,F3597,F3579,F3611,F3553,F3543,F3699,F3531,F3521,F3651,F3629,F3559,F3546,F3534,F3524)}
    //Demand竞争 <<<== SUCCESS 共5条
    //评分3: K:0x600023d9f0d0_F3994[M1{↑饿-16},A3955(向90,距13,皮果),A3958(距89,向18,棒),A3961(向87,距27,棒),A3962(向90,距13,果),飞↑,A3970(向164,距71,棒),A3971(向90,距5,果),A3976(距73,向166,棒),A3979(吃1)] => V:0分
    //评分1: 因actNo直接评最小分: K:0x600037e17de0_F9103[M1{↑饿-16},A4227(向91,距13,皮果)]
    //评分3: K:0x600023e6a5d0_F4206[M1{↑饿-16},A4205(距24,棒),A4204(距12,果),A4203(向86,果),飞↑] => V:-2147483648分
    //第5步 RAnaylst匹配成功:750
    //第6步 R排除Status无效的:749
    //第7步 R排除Infected传染掉的:749
    //第8步 R排除FRSTime来不及的:745
    //R0. I<F3597 F3993[↑饿-16,4果皮,4棒,4棒,4果,飞↑,4棒,4果,4棒,吃]> {0 = 0;} {9 = S0P16;5 = S0P19;1 = S3P20;10 = S0P2;6 = S0P16;2 = S0P19;7 = S0P16;3 = S0P19;8 = S0P16;4 = S0P19;} H1N0:(分:1.00) [CUT:0=>TAR:10]
