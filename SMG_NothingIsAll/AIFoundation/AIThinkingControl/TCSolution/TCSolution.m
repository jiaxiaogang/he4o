...
+(void) hSolution:(HDemandModel*)hDemand{
    ...
    //1. 短时记忆树里的任何cutIndex已发生的部分,都可用于帮助cHav取解决方案;
    //2. cHav结果做为H任务;
    //3. 然后将HFo的末位,传到regroup(),进行识别反思;
    //  a. 目前已经支持在feedback反馈后,传到regroup;
    //  b. 对于反馈前,取到hSolution后,在行为化前,是否先进行regroup反思 (这个要考虑下);
    //4. 此处HN内类比先不废弃,先这么写,等后面再考虑废弃之 (参考24171-3);
    //  a. 分析下rSolution是否做了N的事,而hSolution需要做H的事 (它们的本质是推进和阻止);
    //  b. 如果成立,那么此处取hSolution应该参考rSolution的方式;
    //5. 可将当前瞬时记忆序列做为mask进行cHav联想 (比如在家时,不会想到点外卖,在工作地就首先想到外卖);
    //6. 可以考虑对curAlg的具象生成H任务,比如找武器时没武器,就可想到拿刀,然后再hSolution为厨房拿刀,而无法直接根据武器就想到厨房;
    
    //取hSolution方式:
    //方案1: 参考rSolution,得到取hSolution的方式;
    //  a. 并尝试迭代rSolution(与N契合模型分析),
    //方案2: 以curMask和cutIndex做参考联想,并顺着抽到具象的优先级,逐个联想H解决方案;
    //  a. 问题: 再不脱离场景,也毕竟脱离了场景 (因为这条联想路径,必然是先找到hAlg,再取hFo的);
    //  b. 不如直接在场景内,找到mIsC的alg (当然这一方案即方案一,也要分析它的模型图,评估可行性);
