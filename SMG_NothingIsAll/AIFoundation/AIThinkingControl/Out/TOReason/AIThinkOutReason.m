 ...
 *      2021.11.11 - V5迭代-废弃dsFo (参考24101-第3阶段);
 */
...
-(BOOL) tor_OPushM:(DemandModel*)demand latestMModel:(AIShortMatchModel*)latestMModel{
    ...
    for (TOAlgModel *waitModel in waitModels) {
        ...
        if (isGL) {
            ...
            AnalogyType waitType = bFo.content_p.type;//DS2ATType(bFo.content_p.dataSource);
 ...
 *      2021.10.27: 更新
 *                  1. alg行为化时,当瞬时中M与其有抽具象关系时,转PM进行修正;
 *                  2. tor_OPushM中,当H类型返回,或ActYes返回,或行为输出返回,转PM继续进行不相符部分的修正;
 *                  3. 当一条toValue完成finish时,对它的baseAlg继续进行PM;
 *  @todo
