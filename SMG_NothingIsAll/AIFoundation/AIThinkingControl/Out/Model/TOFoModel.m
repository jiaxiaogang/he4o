...
-(BOOL) step1_CheckFeedbackTORIsValid:(NSArray*)feedbackMatchAlg_ps protoAlg:(AIKVPointer*)protoAlg_p {
    ...
    NSLog(@"aaaaa 等待反馈中:%@ 匹配:%d baseCansetFrom:%@ 状态:%@",Pit2FStr(cansetToSimple.alg_p),mIsC,ShortDesc4Pit(self.cansetFo),CansetStatus2Str(self.cansetStatus));
...
-(void) step2_FeedbackThenCreateHCanset:(AIKVPointer*)protoAlg_p {
    ...
    if (!self.isH && self.cansetStatus != CS_None) {
        ...
        if (ARRISOK(order)) {
            ...
            NSLog(@"aaaaaCanset演化> NewHCanset:%@ toScene:%@ 在%ld帧:A%ld",Fo2FStr(newHCanset),ShortDesc4Node(rCanset),self.cansetActIndex,actIndexAlg_p.pointerId);
        }
    }
    ...
    if (self.isH && self.cansetStatus != CS_None) {
        ...
        if (isEndFrame) {
            ...
            if (orders.count > 1) {
                ...
                NSLog(@"aaaaaCanset演化> AbsHCanset:%@ toScene:%@ 在%ld帧",Fo2FStr(absCansetFo),ShortDesc4Node(sceneTo),targetFoModel.cansetActIndex);
