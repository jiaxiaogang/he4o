...
-(void) lazyInitActionFo {
    if (self.baseSceneModel) {
        //1. 判断包含空概念;
        AIFoNodeBase *contentFo = [SMGUtils searchNode:self.content_p];
        if ([AINetUtils foHasEmptyAlg:self.content_p]) {
            
            //2. 取具象一级cansets (用空概念经验的具象,与当前场景的overrideCansets取交集得出);
            NSArray *conCansets = Ports2Pits([AINetUtils conPorts_All:contentFo]);
            NSArray *overrideCansets = [self.baseSceneModel overrideCansets];
            ...
            self.actionFo_p = firstStrongFo_p;
        }
