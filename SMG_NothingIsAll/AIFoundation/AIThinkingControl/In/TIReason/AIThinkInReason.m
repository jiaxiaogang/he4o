...
+(void) tir_OPushM:(AIShortMatchModel*)newInModel{
    ...
    for (AIShortMatchModel *inModel in inModels) {
        for (AIMatchFoModel *waitModel in inModel.matchPFos) {
            ...
            //查FZ31-1日志;
            //1. 投前看木棒: 第一次构建IRT触发器: F57[纯木棒], F4[纯木棒], F28[纯木棒], F27[带左飞]
            //2. 投出后左飞: 第二次构建IRT触发器: F27[带左飞], F84[带左飞], F56[带左飞]
            //3. 飞后看木棒: 第三次构建IRT触发器: F57[纯木棒], F28[纯木棒]
            //4. R任务对F4决策,并又输出左飞;
            //5. 此时同时撞到,触发痛感,tipOPushM: F57,F4,F28,F27,F84,F56 全正向反馈,即这些全成了S;
            //6. 左飞识别预测: 第四次构建IRT触发器: F338[带左飞], F335[带左飞], F27[带左飞], F339[带左飞], F84[带左飞], F56[带左飞]
            //7. 飞后看木棒: 第五次构建IRT触发器: F57[纯木棒]
            
            //8. 自检9触发:F335,F338,F27,F339,F84,F56
            //9. 分析发现6次自检9,全是来自第6步第4次IRT触发器构建,即全是因为当时已经撞过了,所致;
            
            //////此处主要问题在于,左飞时正好撞到,做处理,如果撞到,说明已经失败,左飞后的一切论为失败;画流程图分析下,这里的矛盾怎么解决;
