 ...
 *  MARK:--------------------单条matchFo模型--------------------
 *  @version
 *      2021.06.29: 将cutIndex拆分为cut和action两个,即新增actionIndex已发生截点 (参考23152);
 */
@class AIFoNodeBase;
 ...
 *  @caller
 *      1. 当为瞬时识别时,cutIndex与已发生actionIndex同值 (因为瞬时时,判断的本来就是当前已经发生的事);
 *      2. 当为反思识别时,cutIndex与已发生actionIndex不同值 (因为反思是一种假设,并判断假设这么做会怎么样);
 */
@property (assign, nonatomic) NSInteger cutIndex;

/**
 *  MARK:--------------------已发生截点--------------------
 *  @desc 已发生与预测的截点 (0开始,已发生含actionIndex);
 */
