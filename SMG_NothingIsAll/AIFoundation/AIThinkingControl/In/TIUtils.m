...
+(void) partMatching_FoV1Dot5:(AIFoNodeBase*)maskFo except_ps:(NSArray*)except_ps decoratorInModel:(AIShortMatchModel*)inModel findCutIndex:(NSInteger(^)(AIFoNodeBase *matchFo,NSInteger lastMatchIndex))findCutIndex{
    ...
    //时序识别结果只做三件事: 学习,预测和新需求;
    //1.1 理性学习; (建议较全含的)
    //      > 非全含的,比如[右上飞]预测了危险,就和proto类比,很片面,且常发;
    //1.2 感性学习; (有mv指向,无论匹配度是多少,感性预测都ok);
    
    //2.1 理性预测; (非全含匹配)
    //      > 全含的不存在理性预测;
    //2.2 感性预测; (有mv指向,无论匹配度是多少,感性预测都ok);
    
    //3. 新增需求; (有mv指向,无论匹配度是多少,感性任务都ok);
    
    //综上: 感性学习和感性预测,以及新增需求,不需要改动;
    //1. 而理性学习需要筛选出全含;
    //2. 理性预测需要筛选出非全含;
