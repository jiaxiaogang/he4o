...
+(TCResult*) action:(TOFoModel*)foModel{
    ...
    if (foModel.cutIndex < foModel.targetIndex - 1) {
        NSLog(@"_Fo行为化第 %ld/%ld 个: %@",foModel.cutIndex+1,foModel.targetIndex,Fo2FStr(curFo));
        ...
        DebugE();
        NSLog(@"_Fo行为化: Finish %ld/%ld 到ActYes",foModel.cutIndex+1,(long)curFo.count);
