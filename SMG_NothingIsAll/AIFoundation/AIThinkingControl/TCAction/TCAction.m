...
+(TCResult*) action:(TOFoModel*)foModel{
    ...
    if (foModel.cansetCutIndex < foModel.targetIndex - 1) {
        NSLog(@"_Fo行为化第 %ld/%ld 个: %@",foModel.cansetCutIndex+1,foModel.targetIndex,Fo2FStr(curFo));
        ...
        DebugE();
        NSLog(@"_Fo行为化: Finish %ld/%ld 到ActYes",foModel.cansetCutIndex+1,(long)curFo.count);
