...
+(AIGroupValueNode*) createGroupValueNode:(NSArray*)content_ps conNodes:(NSArray*)conNodes at:(NSString*)at ds:(NSString*)ds isOut:(BOOL)isOut {
    ...
    AIGroupValueNode *result = nil;
    
    //2. 去重找本地 (仅抽象);
    AIGroupValueNode *localResult = [AINetIndexUtils getAbsoluteMatching_General:content_ps sort_ps:content_ps except_ps:nil getRefPortsBlock:^NSArray *(AIKVPointer *item_p) {
    ...
    for (AIGroupValueNode *checkNode in conNodes) {
    ...
    if (!result) {
        for (AIGroupValueNode *conNode in conNodes) {
    ...
    if (!result) {
        absIsNew = true;
        result = [[AIGroupValueNode alloc] init];
        result.pointer = [SMGUtils createPointerForGroupValue:at dataSource:ds isOut:isOut];
        ...
        NSLog(@"构建新组码:G%ld fromConAlgs:%@",result.pointer.pointerId,CLEANSTR([SMGUtils convertArr:conNodes convertBlock:^id(AINodeBase *obj) {
            return STRFORMAT(@"G%ld",obj.pId);
        }]));
    }
