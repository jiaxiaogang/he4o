...
-(AIAlgNodeBase*) getAbsoluteMatchingAlgNodeWithValueP:(AIPointer*)value_p{
    ...
    if (value_p) {
        result = [self getAbsoluteMatchingAlgNodeWithValuePs:@[value_p] exceptAlg_p:nil isMem:true];
        if (!result) {
            result = [self getAbsoluteMatchingAlgNodeWithValuePs:@[value_p] exceptAlg_p:nil isMem:false];
        }
    }
    return result;
}
