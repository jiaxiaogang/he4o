...
-(void) throw:(CGFloat)hitTime hitBlock:(BOOL(^)())hitBlock{
    ...
    [UIView animateWithDuration:hitTime delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
        self.x += ScreenWidth * (hitTime / ThrowTime);
        ...
        ///4. 后段动画,撞击后部分;
        if (finished) {
            [UIView animateWithDuration:ThrowTime - hitTime delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
                self.x += ScreenWidth * ((ThrowTime - hitTime) / ThrowTime);
            } completion:^(BOOL finished) {
